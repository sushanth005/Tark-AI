  ...
  return (
    <>
      <main className="flex-1 max-w-7xl mx-auto mt-8 w-full grid grid-cols-1 lg:grid-cols-3 gap-8 px-4 sm:px-6 mb-12 relative z-10 animate-fade-in">
        ...
        <div className="lg:col-span-1 bg-white dark:bg-slate-900/50 backdrop-blur-xl p-6 rounded-3xl border border-slate-200 dark:border-slate-800/50 shadow-sm dark:shadow-xl h-fit relative overflow-hidden animate-slide-up">
          {/* Subtle gradient overlay */}
          <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-100 dark:bg-indigo-500/10 blur-[50px] rounded-full pointer-events-none transition-colors duration-1000"></div>

          <h3 className="text-xl font-bold text-slate-900 dark:text-slate-200 mb-6 flex items-center gap-2">
            <span className="w-8 h-8 rounded-xl bg-indigo-100 dark:bg-indigo-500/10 flex items-center justify-center text-indigo-600 dark:text-indigo-400">
              <UploadCloud size={18} />
            </span>
            Run Evaluation
          </h3>
          <form onSubmit={handleAnalyze} className="space-y-5 relative z-10">
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-400 mb-2">Resume Document (PDF)</label>
              <div className="border-2 border-dashed border-slate-300 dark:border-slate-700/50 bg-slate-50 dark:bg-slate-950/50 rounded-2xl p-6 text-center cursor-pointer hover:border-indigo-400 dark:hover:border-indigo-500/50 hover:bg-indigo-50 dark:hover:bg-indigo-500/5 transition relative group">
                ...
                <UploadCloud className="mx-auto text-slate-400 dark:text-slate-500 group-hover:text-indigo-500 dark:group-hover:text-indigo-400 transition mb-3" size={32} />
                <p className="text-sm text-slate-600 dark:text-slate-400 font-medium">{file ? file.name : "Click or drag to drop files"}</p>
              </div>
            </div>

            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-400 mb-2">Target Job Role</label>
              <input type="text" required value={jobRole} onChange={e => setJobRole(e.target.value)} placeholder="e.g. Senior Software Engineer" className="w-full p-3 bg-slate-50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-slate-900 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-600 mb-4 transition" />

              <label className="block text-sm font-medium text-slate-700 dark:text-slate-400 mb-2">Target Job Description</label>
        ...
        <div className="lg:col-span-2 space-y-8 animate-slide-up" style={{ animationDelay: '200ms' }}>
          {result ? (
            <div className="space-y-6">
              ...
              <div ref={reportRef} className="bg-white dark:bg-slate-900/50 backdrop-blur-xl p-8 rounded-3xl border border-slate-200 dark:border-slate-800/50 shadow-sm dark:shadow-xl space-y-8 relative overflow-hidden text-slate-900 dark:text-slate-200">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {Object.entries(result.scoreBreakdown).map(([key, value]) => (
                        <div key={key} className="bg-slate-50 dark:bg-slate-950/50 p-4 rounded-xl border border-slate-200 dark:border-slate-800/80">
                  ...
                  <div className="pt-4 border-t border-slate-200 dark:border-slate-800/50">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {result.keywordDensity.map((kw, i) => (
                        <div key={i} className="flex items-center bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 rounded-lg overflow-hidden">
                          <span className="px-3 py-1.5 text-xs font-medium text-slate-700 dark:text-slate-300">{kw.keyword}</span>
                          <span className="px-2 py-1.5 bg-indigo-50 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 text-xs font-bold border-l border-slate-200 dark:border-slate-700/50">{kw.count}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {result.rewriteSuggestions && result.rewriteSuggestions.length > 0 && (
                  <div className="pt-4 border-t border-slate-200 dark:border-slate-800/50">
                    <h4 className="text-sm font-bold text-slate-900 dark:text-slate-200 uppercase tracking-wider mb-4">Resume Rewrite Suggestions</h4>
                    <div className="space-y-4">
                      {result.rewriteSuggestions.map((suggestion, i) => {
                        ...
                        return (
                          <div key={i} className="bg-slate-50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800/80 p-4 rounded-xl space-y-3">
                            <div className="flex flex-col gap-1">
                              <span className="text-xs font-bold text-rose-600 dark:text-rose-400 uppercase tracking-wider">Before</span>
                              <p className="text-sm text-slate-600 dark:text-slate-400 italic">"{before || suggestion}"</p>
                            </div>
                            {after && (
                              <div className="flex flex-col gap-1">
                                <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">After</span>
                                <p className="text-sm text-slate-800 dark:text-slate-300 font-medium">"{after}"</p>
                              </div>
                            )}
                          </div>
                        )
            ...
            <div className="flex flex-col items-center justify-center h-full min-h-[400px]">
              <Link to="/history" className="flex items-center gap-3 bg-white dark:bg-slate-900/50 hover:bg-slate-50 dark:hover:bg-slate-800/80 text-slate-700 dark:text-slate-300 px-8 py-4 rounded-2xl text-base font-medium shadow-sm dark:shadow-xl transition border border-slate-200 dark:border-slate-800/50">
                <HistoryIcon size={20} className="text-indigo-600 dark:text-indigo-400" /> View Past Assessments
              </Link>
            </div>
          )}
        </div>
